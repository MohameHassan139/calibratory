...
// ── Infusion Pump Summary Screen ──────────────────────────────────────────────

class InfusionSummaryScreen extends StatefulWidget {
  const InfusionSummaryScreen({super.key});

  @override
  State<InfusionSummaryScreen> createState() => _InfusionSummaryScreenState();
}

class _InfusionSummaryScreenState extends State<InfusionSummaryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (session.id != null) return _InfusionCompletedView(session: session);

    final flowRows = session.infusionFlowRows;
    final occRows  = session.infusionOcclusionRows;
            ...
            Obx(() => _ctrl.isLoading.value
                ...
                : SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      onPressed: () => _ctrl.completeInfusionCalibration(
  ...
  Widget _flowTable(List<MeasurementRow> rows) {
    const headers = ['Setting\n(mL/hr*)', 'Avg\nMeasured**', 'Error***',
        'Accepted\nRange ±9%****', 'Status', 'Uncertainty\n(Type A)'];
    const widths  = [80.0, 80.0, 65.0, 130.0, 60.0, 90.0];
        ...
        child: Table(
          ...
          children: [
            ...
            ...List.generate(rows.length, (i) {
              ...
              final errPct  = avg != null && setting > 0
                  ? '${((setting - avg).abs() / setting * 100).toStringAsFixed(2)}%'
                  : 'N/A';
              final rangeStr = i < ranges.length
                  ? '(${ranges[i][0]}  -  ${ranges[i][1]})'
                  : 'N/A';
              final sta = r.status == true ? 'PASS' : r.status == false ? 'FAIL' : 'N/A';
              final staColor = r.status == true ? AppColors.success
  ...
  Widget _occTable(List<OcclusionRow> rows) {
    const headers  = ['Measurement', 'Avg Measured', 'Accepted Range', 'Status', 'Uncertainty\n(Type A)'];
    const widths   = [160.0, 90.0, 130.0, 60.0, 90.0];
...
// ── Infusion Completed View ───────────────────────────────────────────────────

class _InfusionCompletedView extends StatelessWidget {
  final dynamic session;
  const _InfusionCompletedView({required this.session});
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Infusion pump calibration complete. Certificate generated.',
                  style: TextStyle(color: AppColors.textSecondary),
                  textAlign: TextAlign.center),
